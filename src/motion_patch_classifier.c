...
void train_motion_patch_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
	...
	args.d = &buffer;
	args.type = PATCH_CLASSIFICATION_DATA_WITH_MOTION;
	args.threads = 8;
...
void validate_motion_patch_classifier(char* datacfg, char* cfgfile, char* weightfile, float iter_in_k, char* pr_rc_filename, int vis_detections, int save_detections) {
	int i, j, c, index;
	...
	while (count < m) {
		...
		if (*(tokens + 0) && *(tokens + 1)) {
			im = load_image_color_with_motion(*(tokens + 0), *(tokens + 1), 0, 0);			
			free(*(tokens + 0));
			free(*(tokens + 1));
		}
		free(tokens);
...
void validate_motion_patch_classifier_globally(char* datacfg, char* cfgfile, char* weightfile, float iter_in_k, char* pr_rc_filename, int vis_detections, int save_detections) {
	int i, j, c, index;
...
void test_motion_patch_classifier(char* datacfg, char *cfgfile, char* weightfile, char *filename) {
	network net = parse_network_cfg(cfgfile);
	if (weightfile) {
		load_weights(&net, weightfile);
	}
...
void run_motion_patch_classifier(int argc, char **argv)
{
	...
	if (0 == strcmp(argv[2], "test")) test_motion_patch_classifier(datacfg, cfg, weights, filename);
	else if (0 == strcmp(argv[2], "train")) train_motion_patch_classifier(datacfg, cfg, weights, gpus, ngpus, clear);
	else if (0 == strcmp(argv[2], "valid")) validate_motion_patch_classifier(datacfg, cfg, weights,iter_in_k,pr_rc_filename,vis_detections,save_detections);
	else if (0 == strcmp(argv[2], "valid_global")) validate_motion_patch_classifier_globally(datacfg, cfg, weights, iter_in_k, pr_rc_filename, vis_detections, save_detections);
