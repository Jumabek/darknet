...
{
	const int person_cls_id[] = { 0 };
	
	const int animal_cls_ids[] = { 15,16,17,18,19,20,21,22,23};
	
	const int bird_cls_id[] = { 14 };
	...
	int *selected_classes = NULL;
	int num_sel_cls = 0;

	if (l.classes == NUM_COCO_CLS) {
		switch (l.cls_mode) {
		case 0: 
			selected_classes = person_cls_id; 
			num_sel_cls = 1;
			break;
		case 1:
			selected_classes = animal_cls_ids;
			num_sel_cls = 9;
			break;
		case 2:
			selected_classes = bird_cls_id;
			num_sel_cls = 1;
			break;
		case 3:
			selected_classes = others;
			num_sel_cls = NUM_COCO_CLS - (1 + 9 + 1);
			break;
		case 4:
			selected_classes = all;
			num_sel_cls = NUM_COCO_CLS;
			break;
		default:			
			error("When using COCO names, region_layer needs cls_mode");
		}
	}
        ...
        for(n = 0; n < l.n; ++n){
            ...
            if(l.softmax_tree){
                ...
                for(j = 0; j < l.classes; ++j){
					...
					else if (l.classes == NUM_COCO_CLS) {
						int c;
						int flag = 0;
						for (c = 0; c < num_sel_cls; c++) {
							if (selected_classes[c] == j){
								probs[index][j] = (prob > thresh) ? prob : 0;
								flag = 1;
							}													
						}
