...
#define DOABS 1
#define AMBIGUOUS_CLS_ID -1
region_ambiguous_layer make_region_ambiguous_layer(int batch, int w, int h, int n, int classes, int coords)
{
    region_ambiguous_layer l = {0};
    l.type = REGION_AMBIGUOUS;
    ...
    l.forward = forward_region_ambiguous_layer;
    l.backward = backward_region_ambiguous_layer;
#ifdef GPU
    l.forward_gpu = forward_region_ambiguous_layer_gpu;
    l.backward_gpu = backward_region_ambiguous_layer_gpu;
...
box get_region_ambiguous_box(float *x, float *biases, int n, int index, int i, int j, int w, int h)
{
    box b;
...
float delta_region_ambiguous_box(box truth, float *x, float *biases, int n, int index, int i, int j, int w, int h, float *delta, float scale)
{
    box pred = get_region_ambiguous_box(x, biases, n, index, i, j, w, h);
...
void delta_region_ambiguous_class(float *output, float *delta, int index, int class, int classes, float scale, float *avg_cat)
{
	if (class == AMBIGUOUS_CLS_ID) return;
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
					...
					box pred = get_region_ambiguous_box(l.output, l.biases, n, index, i, j, l.w, l.h);
                    float best_iou = 0;
                    int best_class = -2;
                    for(t = 0; t < 30; ++t){
                    ...
                    if(l.classfix == -1) l.delta[index + 4] = l.noobject_scale * ((best_iou - l.output[index + 4]) * logistic_gradient(l.output[index + 4]));
                        ...
                        if (best_iou > l.thresh) {
                            l.delta[index + 4] = 0;
                            if(l.classfix > 0){
                                delta_region_ambiguous_class(l.output, l.delta, index + 5, best_class, l.classes, l.class_scale*(l.classfix == 2 ? l.output[index + 4] : 1), &avg_cat);
                                ++class_count;
                            }
                        }
                    }
                    ...
                    if(*(state.net.seen) < 12800){
                        ...
                        delta_region_ambiguous_box(truth, l.output, l.biases, n, index, i, j, l.w, l.h, l.delta, .01);
                    }
                }
            }
        }
        for(t = 0; t < 30; ++t){
			...
			if (class == AMBIGUOUS_CLS_ID) continue;
            ...
            float iou = delta_region_ambiguous_box(truth, l.output, l.biases, best_n, best_index, i, j, l.w, l.h, l.delta, l.coord_scale);
            if(iou > .5) recall += 1;
            avg_iou += iou;
            ...
            delta_region_ambiguous_class(l.output, l.delta, best_index + 5, class, l.classes, l.class_scale, &avg_cat);
            ++count;
            ++class_count;
        }
    }
...
void get_region_ambiguous_boxes(layer l, int w, int h, float thresh, float **probs, box *boxes, int only_objectness, int *map)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            boxes[index] = get_region_ambiguous_box(predictions, l.biases, n, box_index, col, row, l.w, l.h);
            boxes[index].x *= w;
            boxes[index].y *= h;
            boxes[index].w *= w;
            boxes[index].h *= h;
...
void forward_region_ambiguous_layer_gpu(const region_ambiguous_layer l, network_state state)
{
    ...
    forward_region_ambiguous_layer(l, cpu_state);
