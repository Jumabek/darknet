...
void delta_region_class_focal(float *output, float *delta, int index, int class, int classes, tree *hier, float scale, float *avg_cat)
{
	int i, n;
	if (hier) {
		float pred = 1;
		...
		*avg_cat += pred;
		...
		for (n = 0; n < classes; ++n) {
			delta[index + n] = scale * (((n == class) ? 1 : 0) - output[index + n]);
			delta[index + n] = delta[index + n] * (delta[index + n]*delta[index + n]); //from https://arxiv.org/pdf/1708.02002.pdf)
			if (n == class) *avg_cat += output[index + n];
		}
	}
}
